import Group.{IntPlus, MultiModulo11}
...
class Modulo11AdvancedTest extends TestKit(ActorSystem("testSystem"))
  with WordSpecLike
  with MustMatchers
  with ImplicitSender
{
  "A Root actor" must {
    ...
    "receive messages" in {
      ...
      node_one ! SendAggregate()
      within(200 millis) {
        underLyingRootActor.getAggregateMass must equal(3)
      }
    }
  }
