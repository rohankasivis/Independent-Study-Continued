  ...
  with ImplicitSender
{
    "The GAPUtil test cases" must {
        ...
        // more nodes added here for restore_table_invariant tests
        val node_four = TestActorRef(new GAPNode[Int](monoid))
        val node_five = TestActorRef(new GAPNode[Int](monoid))
        val node_six = TestActorRef(new GAPNode[Int](monoid))
        val node_seven = TestActorRef(new GAPNode[Int](monoid))
        val node_eight = TestActorRef(new GAPNode[Int](monoid))
        ...
        "test for parent_min_val: check within tables that do not have parents (in root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.setStatus(root_table, node_three, Self())
            root_table = utils_use.update_entry(root_node, root_node_second, 10, 0, node_three, root_table)
            root_table = helpers_use.parent_min_val(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val parent:ActorRef = utils_use.getParent(root_table).get
            root_table.get(parent).get.get_level().get must equal (minimum)
        }
        "test for parent_min_val: check within tables that do not have parents (in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
        ...
        "test for parent_min_val: check for case where all elements have level null (in root) and no parents" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.remove_entry(root_node, root_table)
            root_table = utils_use.remove_entry(root_node_second, root_table)
            helpers_use.get_minimum(root_table) must equal (None)
            root_table = helpers_use.parent_min_val(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val parent:ActorRef = utils_use.getParent(root_table).get
            root_table.get(parent).get.get_level().get must equal (minimum)
        }
        "test for parent_min_val: check for case where all elements have level null (in nonroot) and no parents" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
        ...
        "test for parent_min_val: check for case where all elements have level null (in root) and one parent" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for parent_min_val: check for case where all elements have level null (in nonroot) and one parent" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.remove_entry(node_one, one_table)
        ...
        "test for handle_self_level: check for self status in a table where minimum + 1 does not have self (in root: test 1)" in {
            ...
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.setStatus(root_table, node_three, Self())
            root_table = utils_use.update_entry(root_node_second, root_node_second, 10, 1, node_three, root_table)
            root_table = helpers_use.handle_self_level(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val self:ActorRef = utils_use.getSelf(root_table).get
            root_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for handle_self_level: check for self status in a table where minimum + 1 does not have self (in root: test 2)" in {
            ...
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.update_entry(node_one, root_node_second, 10, 2, root_node_second, root_table)
            root_table = utils_use.setStatus(root_table, node_three, Self())
            root_table = utils_use.update_entry(root_node_second, root_node_second, 10, 1, node_three, root_table)
        ...
        "test for handle_self_level: check for self status in a table where minimum + 1 does not have self (in nonroot: test 1)" in {
            ...
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.update_entry(node_two, root_node_second, 10, 2, node_three, one_table)
            one_table = utils_use.setStatus(one_table, node_three, Self())
            one_table = helpers_use.handle_self_level(one_table)
        ...
        "test for handle_self_level: check for self status in table which has no self (in root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.update_entry(node_one, root_node_second, 10, 1, node_three, root_table)
            root_table = utils_use.remove_entry(root_node_second, root_table)
            root_table = helpers_use.handle_self_level(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val self:ActorRef = utils_use.getSelf(root_table).get
        ...
        "test for handle_self_level: check for self status in table which has no self (in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.remove_entry(node_one, one_table)
            one_table = utils_use.update_entry(node_two, root_node_second, 10, 0, node_three, one_table)
            one_table = utils_use.update_entry(node_three, root_node_second, 10, 1, node_two, one_table)
            one_table = helpers_use.handle_self_level(one_table)
            val minimum:Int = helpers_use.get_minimum(one_table).get
            val self:ActorRef = utils_use.getSelf(one_table).get
            one_table.get(self).get.get_level().get must equal (minimum + 1)
        }
        "test for handle_self_level: check for case where only one level exists and no self (in root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for handle_self_level: check for case where only one level exists and no self (in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
        ...
        "test for handle_self_level: check for case where only one level exists and one self (in root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for handle_self_level: check for case where only one level exists and one self (in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.setStatus(one_table, node_one, Par())
        ...
        "test for handle_self_level: check for case where all None levels (in root)" in {
            // order will always be calling parent_min_val and then handle_self_level, but make sure it works here and the next case
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for handle_self_level: check for case where all None levels (in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.remove_entry(node_one, one_table)
        ...
        "test for handle_self_level: no minimum + 1 level exists (case 1: root)" in {
            ...
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.setLevel(root_table, root_node_second, Some(2))
            root_table = helpers_use.handle_self_level(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val self:ActorRef = utils_use.getSelf(root_table).get
            root_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for handle_self_level: no minimum + 1 level exists (case 2: root)" in {
            ...
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.setLevel(root_table, node_one, Some(2))
            root_table = helpers_use.handle_self_level(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val self:ActorRef = utils_use.getSelf(root_table).get
            root_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for handle_self_level: no minimum + 1 level exists (case 3: root)" in {
            ...
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.remove_entry(root_node_second, root_table)
            root_table = utils_use.setLevel(root_table, node_one, Some(3))
            root_table = utils_use.setLevel(root_table, node_two, Some(3))
            root_table = helpers_use.handle_self_level(root_table)
            val minimum:Int = helpers_use.get_minimum(root_table).get
            val self:ActorRef = utils_use.getSelf(root_table).get
            root_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for handle_self_level: no minimum + 1 level exists (case 1: nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, true, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, true, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, true, one_table)
            one_table = utils_use.setStatus(one_table, node_two, Par())
            one_table = utils_use.setLevel(one_table, node_two, Some(3))
            one_table = helpers_use.handle_self_level(one_table)
            val minimum:Int = helpers_use.get_minimum(one_table).get
            val self:ActorRef = utils_use.getSelf(one_table).get
            one_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for handle_self_level: no minimum + 1 level exists (case 2: nonroot)" in {
            ...
            one_table = utils_use.new_entry(node_three, root_node_second, true, one_table)
            one_table = utils_use.setStatus(one_table, node_one, Par())
            one_table = utils_use.setLevel(one_table, node_two, Some(5))
            one_table = helpers_use.handle_self_level(one_table)
            val minimum:Int = helpers_use.get_minimum(one_table).get
            val self:ActorRef = utils_use.getSelf(one_table).get
            one_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for handle_self_level: no minimum + 1 level exists (case 3: nonroot)" in {
            ...
            one_table = utils_use.setStatus(one_table, node_one, Par())
            one_table = utils_use.setLevel(one_table, node_two, Some(5))
            one_table = utils_use.setLevel(one_table, node_three, Some(6))
            one_table = helpers_use.handle_self_level(one_table)
            val minimum:Int = helpers_use.get_minimum(one_table).get
            val self:ActorRef = utils_use.getSelf(one_table).get
            one_table.get(self).get.get_level().get must equal (minimum + 1)
        ...
        "test for confirm_one_self: more than one self exists (test 3: in root)" in {
            ...
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.setStatus(root_table, node_one, Self())
            root_table = utils_use.setStatus(root_table, node_two, Self())
            root_table = utils_use.setStatus(root_table, node_three, Self())
            root_table = utils_use.setStatus(root_table, root_node, Self())
            root_table = utils_use.setLevel(root_table, node_one, Some(4))
            root_table = utils_use.setLevel(root_table, node_two, Some(4))
            root_table = utils_use.setLevel(root_table, node_three, Some(4))
            root_table = helpers_use.parent_min_val(root_table)
            root_table = helpers_use.confirm_one_parent(root_table)
            root_table = helpers_use.handle_self_level(root_table)
            root_table = helpers_use.confirm_one_self(root_table)
        ...
        "test for confirm_one_self: more than one self exists (test 4: in root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for confirm_one_self: more than one self exists (test 5: in root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for confirm_one_self: more than one self exists (test 4: in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.setStatus(one_table, node_two, Self())
        ...
        "test for confirm_one_self: more than one self exists (test 5: in nonroot)" in {
            one_table = utils_use.new_entry(node_one, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_two, root_node_second, false, one_table)
            one_table = utils_use.new_entry(node_three, root_node_second, false, one_table)
            one_table = utils_use.setStatus(one_table, node_two, Self())
            ...
            one_table = utils_use.setStatus(one_table, node_one, Par())
            one_table = utils_use.setLevel(one_table, node_two, Some(3))
            one_table = utils_use.setLevel(one_table, node_three, Some(3))
            one_table = helpers_use.handle_self_level(one_table)
            one_table = helpers_use.confirm_one_self(one_table)
            val second_count:Int = utils_use.getNumSelf(one_table)
            second_count must equal(1)
        ...
        "test for restore_table_invariant: table does not need changes (test 1 for root)" in {
            ...
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.restore_table_invariant(root_table)
        ...
        "test for restore_table_invariant: table does not need changes (test 1 for nonroot)" in {
            ...
            one_table = utils_use.update_entry(node_two, root_node_second, 10, 0, node_three, one_table)
            one_table = utils_use.setStatus(one_table, node_two, Par())
            one_table = utils_use.restore_table_invariant(one_table)
        ...
        "test for restore_table_invariant: table does need changes (test 1 for root)" in {
            // all None levels, no Self or Parent elements
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_four, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_five, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_six, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_seven, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_eight, root_node_second, true, root_table)
            root_table = utils_use.remove_entry(root_node, root_table)
            root_table = utils_use.remove_entry(root_node_second, root_table)
            root_table = utils_use.restore_table_invariant(root_table)
            val result:Boolean = utils_use.everything_works(root_table)
        ...
        "test for restore_table_invariant: table does need changes (test 2 for root)" in {
            ...
            root_table = utils_use.setStatus(root_table, node_two, Par())
            root_table = utils_use.setStatus(root_table, node_three, Par())
            root_table = utils_use.setStatus(root_table, node_four, Par())
            root_table = utils_use.setStatus(root_table, node_five, Par())
            root_table = utils_use.setStatus(root_table, node_six, Par())
            root_table = utils_use.setStatus(root_table, node_seven, Par())
            root_table = utils_use.setStatus(root_table, node_eight, Par())
            root_table = utils_use.restore_table_invariant(root_table)
            val result:Boolean = utils_use.everything_works(root_table)
            result must equal (true)
        }
        "test for restore_table_invariant: table does need changes (test 3 for root)" in {
            ...
            root_table = utils_use.setStatus(root_table, node_two, Self())
            root_table = utils_use.setStatus(root_table, node_three, Self())
            root_table = utils_use.setStatus(root_table, node_four, Self())
            root_table = utils_use.setStatus(root_table, node_five, Self())
            root_table = utils_use.setStatus(root_table, node_six, Self())
            root_table = utils_use.setStatus(root_table, node_seven, Self())
            root_table = utils_use.setStatus(root_table, node_eight, Self())
            root_table = utils_use.restore_table_invariant(root_table)
            val result:Boolean = utils_use.everything_works(root_table)
            result must equal (true)
        }
        "test for restore_table_invariant: table does need changes (test 4 for root)" in {
            // this includes a mixture of both self and par values
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for restore_table_invariant: table does need changes (test 5 for root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
        ...
        "test for restore_table_invariant: table does need changes (test 6 for root)" in {
            // similar to previous but slightly more advanced
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
            ...
            root_table = utils_use.setLevel(root_table, node_two, Some(3))
            root_table = utils_use.setStatus(root_table, node_three, Par())
            root_table = utils_use.setLevel(root_table, node_three, Some(2))
            root_table = utils_use.restore_table_invariant(root_table)
            val result:Boolean = utils_use.everything_works(root_table)
            result must equal (true)
        }
        "test for restore_table_invariant: table does need changes (test 7 for root)" in {
            // all child elements
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            ...
            root_table = utils_use.new_entry(node_eight, root_node_second, true, root_table)
            root_table = utils_use.remove_entry(root_node_second, root_table)
            root_table = utils_use.setStatus(root_table, root_node, Child())
            root_table = utils_use.setStatus(root_table, node_one, Child())
            root_table = utils_use.setStatus(root_table, node_two, Child())
            root_table = utils_use.setStatus(root_table, node_three, Child())
            root_table = utils_use.setStatus(root_table, node_four, Child())
            root_table = utils_use.setStatus(root_table, node_five, Child())
            root_table = utils_use.setStatus(root_table, node_six, Child())
            root_table = utils_use.setStatus(root_table, node_seven, Child())
            root_table = utils_use.setStatus(root_table, node_eight, Child())
            root_table = utils_use.setLevel(root_table, node_one, Some(3))
            root_table = utils_use.setLevel(root_table, node_two, Some(5))
            root_table = utils_use.restore_table_invariant(root_table)
            val result:Boolean = utils_use.everything_works(root_table)
            result must equal (true)
        }
        "test for restore_table_invariant: table does need changes (test 8 for root)" in {
            root_table = utils_use.new_entry(root_node, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_one, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_two, root_node_second, true, root_table)
            root_table = utils_use.new_entry(node_three, root_node_second, true, root_table)
