...
class Root extends NodeActors
{
    ...
    def handle_new(newActor: ActorRef, operations: Group[Int]) = {
      ...
      newActor ! Status(self, Some(0), operations) // passing in a status of level 0 to the send function
      adjacent += newActor
    ...
    def handle_agg_message(aggregateActor: ActorRef, valueToAggregate: Int, operations: Group[Int]) = {
      if (isEnabled)
        println("received Aggregate(" + valueToAggregate + ") from " + aggregateActor.toString())
      aggregate_mass = operations.op(aggregate_mass, valueToAggregate)
      if (isEnabled)
        println("Aggregate Mass value = " + aggregate_mass)
      balance = balance + (aggregateActor -> (operations.op(balance.get(aggregateActor).get, operations.inverse(valueToAggregate)))) // reassignment of received mass to modify index
    }
    ...
    def receive: Receive = {
      ...
      case Status(arg1, arg2, operations) => val result = {
